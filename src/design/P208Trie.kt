...
class P208Trie(var end: Boolean = false) {
    companion object {
        const val R = 26
    }

    private val next = Array<P208Trie?>(R) { null }
    ...
    private fun insert(word: String, node: P208Trie, index: Int) {
        if (index >= word.length) {
            return
        }
        val i = word[index] - 'a'
        if (node.next[i] == null) {
            node.next[i] = P208Trie(index == word.length - 1)
        }
    ...
    private fun search(word: String, node: P208Trie?, prefix: Boolean, index: Int): Boolean {
        if (node == null) {
            return false
        }
